...
export default async function handler(req: any, res: any) {
  ...
  if (req.method === "POST") {
    try {
      ...
      const cleanPhone = (record.user_phone || "").replace(/[^0-9]/g, "");
      const officialName = OFFICIAL_NAMES[cleanPhone] || record.user_name || "User";
      const messageContent = record.content || "";
      const msgType = record.message_type || "text";

      logger.info(
        { cleanPhone, officialName, messageContent },
