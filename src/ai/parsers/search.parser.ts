...
export function extractDeterministicSearchIntent(userQuery: string): SearchIntent | null {
  ...
  if (
    /(?:bagaimana|gimana|tata\s+cara|panduan|cara(?:\s+nya)?)\s+(?:cara(?:nya)?\s+)?(?:input|menginput|catat|mencatat|masuk(?:kan)?|tambah(?:kan)?|tulis|rekam|bikin|buat|pakai|penggunaan)\b/i.test(lower) ||
