...
export function parseIndonesianMonth(raw: string): string {
  const now = new Date();
...
export function extractTransactionDate(text: string): { cleanText: string; date: string } {
...
export class CommandHandler {
  ...
  async handleCommand(
    ...
    if (command === "/pemasukan" || command === "/masuk" || command === "/income" || command === "/tambahsaldo") {
      ...
      else if (lowerKet.includes("jual") || lowerKet.includes("proyek") || lowerKet.includes("order") || lowerKet.includes("kopi")) category = "Pemasukan: Penjualan";
    ...
    if (command === "/tambah" || command === "/izinkan" || command === "/setujui" || command === "/approve") {
      ...
      let autoLid: string | undefined = undefined;
      try {
        ...
        const { data: recentLogs } = await (this.userRepo as any).supabase
          .from("chat_logs")
          .select("user_phone, user_name")
          .order("created_at", { ascending: false })
          .limit(30);

        const matchedLog = (recentLogs || []).find((l: any) => {
