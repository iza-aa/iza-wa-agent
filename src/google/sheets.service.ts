...
export class GoogleSheetsService {
  ...
  async ensureSheetInitialized(sheetId: string = config.GOOGLE_SHEET_ID): Promise<void> {
    try {
      ...
      let targetSheet = existingSheets.find(
      ...
      if (!targetSheet) {
        ...
        const oldSheet = existingSheets.find(
          (s: any) => s.properties?.title === "Data Transaksi"
        );
      ...
      const oldDasbor = existingSheets.find(
        (s: any) => s.properties?.title === "Dasbor"
      );
      if (oldDasbor) {
        try {
          await this.sheetsClient.spreadsheets.batchUpdate({
  ...
  async setupDashboardTab(sheetId: string = config.GOOGLE_SHEET_ID): Promise<void> {
    try {
      const spreadsheet = await this.sheetsClient.spreadsheets.get({
        spreadsheetId: sheetId,
      });
      ...
      const trxSheet = meta.data.sheets?.find((s: any) => s.properties?.title === this.sheetTitle);
  ...
  async formatTransactionRow(rowIndex: number, sheetId: string = config.GOOGLE_SHEET_ID): Promise<void> {
    try {
      const meta = await this.sheetsClient.spreadsheets.get({ spreadsheetId: sheetId });
      const trxSheet = meta.data.sheets?.find((s: any) => s.properties?.title === this.sheetTitle);
      const trxSheetId = trxSheet?.properties?.sheetId || 0;
  ...
  async getHighestTransactionSequence(prefix: string, sheetId: string = config.GOOGLE_SHEET_ID): Promise<number> {
    try {
      const res = await this.sheetsClient.spreadsheets.values.get({
        spreadsheetId: sheetId,
  ...
  async deleteTransactionItems(trxId: string, sheetId: string = config.GOOGLE_SHEET_ID): Promise<boolean> {
    try {
      const dRes = await this.sheetsClient.spreadsheets.values.get({
        spreadsheetId: sheetId,
        range: this.dataRincianTitle + "!A2:I",
      });
      const dRows = (dRes.data.values || []).filter((r: any[]) => r[0] && r[3]);
      const shortId = trxId.replace(/^T\d+-/, "").toUpperCase();
      const keptDRows = dRows.filter((r: any[]) => {
  ...
  async syncFromSheetToDatabase(
    ...
    try {
      ...
      let rincianRows = (rincianRes.data.values || []).filter((r: any[]) => r[0] && r[3]);

      // If transactions were deleted from Transaksi, also purge them from Data_Rincian
      if (idsToDelete.length > 0) {
        const keptRincian = rincianRows.filter((r: any[]) => !idsToDelete.includes((r[1] || "").toString().trim()));
  ...
  async renderRincianBelanjaSheet(sheetId: string = config.GOOGLE_SHEET_ID): Promise<void> {
    try {
      ...
      for (let i = 0; i < dataRows.length; i++) {
        ...
        const isPackagingNote =
          dNotes &&
          dNotes !== "-" &&
          /\b(\d+(\.\d+)?\s*(gantung|biji|bks|bungkus|rak|kotak|bal|ikat|btl|botol|kaleng|ekor|dos|karton|roll|lembar|pax|pack|pcs|grm|gram|ons|kg|liter|unit|buah))\b/i.test(
            dNotes
          );
        if (isPackagingNote) {
          dQty = dNotes;
