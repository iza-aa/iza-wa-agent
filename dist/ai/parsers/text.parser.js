...
Analisis pesan di atas dan kembalikan JSON sesuai instruksi.`;
        ...
        try {
            ...
            let headerDept = null;
            if (/\b(?:keperluan|untuk|buat|divisi|pos|bagian)\s+barista\b/i.test(userText))
                headerDept = "Barista";
            else if (/\b(?:keperluan|untuk|buat|divisi|pos|bagian)\s+dapur\b/i.test(userText))
                headerDept = "Dapur";
            else if (/\b(?:keperluan|untuk|buat|divisi|pos|bagian)\s+waiters?\b/i.test(userText))
                headerDept = "Waiters";
            else if (/\b(?:keperluan|untuk|buat|divisi|pos|bagian)\s+kasir\b/i.test(userText))
                headerDept = "Kasir";
            else if (/\b(?:keperluan|untuk|buat|divisi|pos|bagian)\s+kafe\b/i.test(userText))
                headerDept = "Kafe";
            const normalizedItems = (rawTrx.items || []).map((it) => {
                ...
                if (headerDept) {
                    ...
                    if (/kopi|sirup|syrup|susu|tea|teh|powder|creamer|sedotan|cup/i.test(itemNameLower))
                        dept = "Barista";
                    else if (/ayam|sayur|daging|ikan|minyak|beras|telur|bumbu|bawang|cabe|tomat/i.test(itemNameLower))
                        dept = "Dapur";
                    else if (/sabun|sunlight|pel|tisu|tissue|plastik\s*sampah/i.test(itemNameLower))
                        dept = "Waiters";
                    else if (/thermal|struk|kresek/i.test(itemNameLower))
                        dept = "Kasir";
