...
export class TelegramAssistantBot {
    ...
    setupHandlers() {
        ...
        this.bot.command(["start", "menu", "help"], async (ctx) => {
            ...
            const welcomeText = `👋 *Halo ${name}! Selamat Datang di IZA Executive AI Assistant*\n\n` +
        ...
        this.bot.command("link", async (ctx) => {
            ...
            if (!args) {
                await ctx.reply(`📱 *CARA TAUTKAN NOMOR HP:*\n\nKetik: \`/link [NomorHPAnda]\`\nContoh: \`/link 08123456789\`\n\n_Setelah ditautkan, semua transaksi di Telegram akan otomatis tercatat atas nama Anda di Google Spreadsheet._`, { parse_mode: "Markdown" });
                return;
            }
            ...
            if (user) {
                ...
                await ctx.reply(`🎉 *AKUN BERHASIL DITAUTKAN!*\n\nHalo *${user.name}*, akun Telegram Anda telah resmi terhubung dengan nomor \`+${user.phone_number}\` (${user.role}).\n\nSekarang Anda dapat langsung mencatat transaksi, kirim foto nota, atau tanya laporan kas.`, { parse_mode: "Markdown" });
            }
            else {
                await ctx.reply(`⚠️ Nomor \`+${targetPhone}\` belum terdaftar di database sistem.\n\nPastikan Super Admin telah mendaftarkan nomor Anda terlebih dahulu via \`/tambah ${targetPhone} [NamaAnda]\`.`, { parse_mode: "Markdown" });
            }
        });
