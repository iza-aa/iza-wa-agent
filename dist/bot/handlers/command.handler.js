...
export class CommandHandler {
    ...
    budgetRepo;
    billRepo;
    constructor(userRepo, trxRepo, budgetRepo, billRepo) {
    ...
    async handleCommand(senderPhone, text) {
        ...
        if (command === "/budget" || command === "/anggaran" || command === "/limit") {
            ...
            const monthlySummary = await this.trxRepo.getMonthlySummary(currentMonth);
            const matchedKey = Object.keys(monthlySummary.byCategory || {}).find((k) => k.toLowerCase().includes(category.toLowerCase()) || category.toLowerCase().includes(k.toLowerCase()));
...
export function parseHumanNominal(raw) {
    if (!raw)
        return 0;
