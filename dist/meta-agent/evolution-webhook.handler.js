...
export class EvolutionWebhookHandler {
    ...
    async handleIncomingWebhook(body) {
        ...
        else if (msgContent.listResponseMessage || rawData.listResponseMessage) {
            const listResp = msgContent.listResponseMessage || rawData.listResponseMessage;
            interactiveButtonId = listResp.singleSelectReply?.selectedRowId || listResp.selectedRowId;
            messageText = listResp.title || listResp.singleSelectReply?.selectedRowId || interactiveButtonId || "";
        }
        ...
        try {
            ...
            if (result.buttons && result.buttons.length > 0) {
                await evolutionApiClient.sendInteractiveButtons(senderPhone, result.reply, result.buttons);
            }
            else {
            ...
            await evolutionApiClient.sendTextMessage(senderPhone, "⚠️ Mohon maaf, terjadi kendala teknis saat memproses pesan Anda. Silakan coba sesaat lagi.");
        }
    }
}
export const evolutionWebhookHandler = new EvolutionWebhookHandler();
//# sourceMappingURL=evolution-webhook.handler.js.map