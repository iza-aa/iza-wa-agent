...
export class MetaApiClient {
    ...
    async sendTextMessage(to, messageText) {
        ...
        try {
            ...
            logger.info({ to: cleanTo, messageId: resData?.messages?.[0]?.id }, "Sent text message via Meta API");
            return true;
        }
        catch (err) {
    ...
    async sendInteractiveButtons(to, bodyText, buttons, headerText, footerText) {
        ...
        try {
            ...
            const resData = await response.json();
            if (!response.ok) {
                logger.warn({ resData, status: response.status }, "Failed to send interactive button message, falling back to plain text");
            ...
            logger.info({ to: cleanTo, messageId: resData?.messages?.[0]?.id }, "Sent interactive button message via Meta API");
            return true;
        }
        catch (err) {
    ...
    async markAsRead(messageId) {
        if (!this.token || !this.phoneId || !messageId)
            return false;
        const url = `https://graph.facebook.com/v21.0/${this.phoneId}/messages`;
        try {
            await fetch(url, {
